...
class RandomForest(Classifier):
    ...
    def hps_impl(data=None):
        ...
        dic = {'bootstrap': ['c', [True, False]],
                ...
                'min_samples_leaf': ['z', 1, floor(data.size() / 2)], # Int (# of instances) is better than float (proportion of instances) because different floats can collide to a same int, making intervals of useless real values.
                'min_samples_split': ['z', 2, floor(data.size() / 2)], # Same reason as min_samples_leaf
                'max_depth': ['z', 2, data.size()],
