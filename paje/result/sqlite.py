...
class SQLite(Cache):
    ...
    def get_set(self, data):
        ...
        :return:
        """
        self.cursor.execute("select data from dset where hash=?", [data.__hash__()])
    ...
    def setexists(self, data):
        ...
        :return:
        """
        self.cursor.execute("select count(1) from dset where hash=?", [data.__hash__()])
        rows = self.cursor.fetchall()
    ...
    def argsexist(self, component):
        ...
        :return:
        """
        self.cursor.execute("select count(1) from args where hash=?", [component.__hash__()])
        rows = self.cursor.fetchall()
        return rows is not None and len(rows) > 0
    ...
    def get_model(self, component, train):
        ...
        :return:
        """
        self.cursor.execute("select name, args, dump from model where name=? and args=? and train=?",
    ...
    def get_or_else(self, component, train, f, test=None):
        ...
        self.cursor.execute("create table if not exists out (args BLOB, setin BLOB, setout BLOB)")
        ...
        if model is not None:
            ...
            if not self.setexists(train):
                self.query("insert into dset values (?, ?)",
                           [train.__hash__(), SQLite.pack(train)])
            trainout = f(train)
            ...
            self.query("insert into out values (?, ?, ?)",
                       [component.__hash__(), train.__hash__(), trainout.__hash__()])

            # Store model and result of training data.
            dump = SQLite.pack(component.model)
            self.query("insert into model values (?, ?, ?, ?)",
                       [type(component.__class__()).__name__, component.__hash__(), train.__hash__(), dump])
