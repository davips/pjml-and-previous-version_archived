...
class TsSplit(AbstractSplit, ABC):
    def __init__(self, **kwargs):
        ...
        class Step(withNoInfo, DSStep):  # TODO: info?
            def __init__(self, component: withSerialization, data=None):
                super().__init__(component)
                self.train = data  # Class Component will pass train here, because Step will be understood as Model.

            def _transform_impl(self, data: t.Data) -> t.Result:
                if data != self.train:
                    raise Exception(
                        f"Split needs the same data object at training and test! {data.id} != {self.train.id}"
                    )
